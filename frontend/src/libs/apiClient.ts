...
export async function getItems(
...
): Promise<HttpResult<Item[]> & { nextCursor?: string | null }> {
  ...
  async function inner(
    ...
    if (extra?.cursor) params.push(`cursor=${encodeURIComponent(extra.cursor)}`);
    ...
    return { ...res, data: list, meta, nextCursor } as any as HttpResult<Item[]> & { nextCursor?: string | null };
  }
