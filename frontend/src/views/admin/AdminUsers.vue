...
<template>
  <div>
    ...
    <SearchList :items="usersStore.users ?? []" :loading="false" placeholder="Поиск по имени или телефону" :filterKeys="['name','phone']">
      <template #default="{ items }">
        <EmptyState v-if="!items.length" text="Пользователей пока нет" />
        <template v-else>
          <RowCard
            v-for="u in items"
            :key="u.id"
